...
namespace Compiler.SyntacticAnalysis
{
    ...
    public class Parser
    {
        ...
        /// <summary>
        /// Parses an Quick if command
        /// </summary>
        /// <returns>An abstract syntax tree representing the command</returns>
        private ICommandNode ParseQuickIfCommand()
        {
            Debugger.Write("Parsing Question mark Command");
            Position startPosition = CurrentToken.Position;
            Accept(QuestionMark);
            IExpressionNode expression = ParseExpression();
            Accept(EqualTo);
            ICommandNode EqualCommand = ParseSingleCommand();
            return new QuickIfCommandNode(expression, EqualCommand, startPosition);
        }

        /// <summary>
        /// Parses an loop command
        /// </summary>
        /// <returns>An abstract syntax tree representing the command</returns>
        private ICommandNode ParseLoopCommand()
        {
            ...
            Accept(While);
            IExpressionNode whileExpression = ParseExpression();
            ICommandNode whileCommand = ParseSingleCommand();
            Accept(Repeat);
            return new LoopCommandNode(loopCommand, whileExpression, whileCommand, startPosition);
        }
        ...
        private IDeclarationNode ParseVarDeclaration()
        {
            ...
            Accept(Var);
            IdentifierNode identifier = ParseIdentifier();
            Accept(Is); // Changed to Is to match with the grammer
